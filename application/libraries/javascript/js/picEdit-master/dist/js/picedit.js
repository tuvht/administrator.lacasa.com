...
;(function ( $, window, document, undefined ) {
    ...
    var pluginName = 'picEdit',
        defaults = {
			...
			redirectUrl: 'http://kiemtraweb.vn/user/myaccount',				// Page url for redirect on form submit
	...
	Plugin.prototype = {
		...
		set_loading: function (message) {
			...
			else return this.set_messagebox("Uploading...", false, false);
		},
		...
		resize_image: function () {
			...
			this.set_loading(1).delay(200).promise().done(function() {
				...
				_this._create_image_with_datasrc(canvas.toDataURL("image/jpeg"), function() {
					_this.hide_messagebox();
				});
			});
			this._hideAllNav();
		...
		take_photo: function() {
			...
			this._create_image_with_datasrc(canvas.toDataURL("image/jpeg"), function() {
		...
		crop_image: function() {
			...
			this.set_loading(1).delay(200).promise().done(function() {
				...
				/*_this._image.src = canvas.toDataURL("image/jpeg");
				_this._resizeViewport();
				_this._paintCanvas();
		...
		crop_open: function () {
			if(!this._image) return this._hideAllNav(1);
            //this._cropping.crop.css({'height':'900px'});
            //this._cropping.cropframe.height="100px";
		...
		_doRotation: function (degrees){
			...
			this._image.src = canvas.toDataURL("image/jpeg");
			this._paintCanvas();
