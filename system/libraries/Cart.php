...
class CI_Cart {
	...
	function update($items = array())
	{
		...
		$save_cart = FALSE;
		if (isset($items['rowid']) AND isset($items['qty']) OR isset($items['price']) OR isset($items['options']))
		{
		...
		else
		{
			foreach ($items as $val)
			{
				if (is_array($val) AND isset($val['rowid']) AND isset($val['qty']) OR isset($val['price']) OR isset($val['options']))
				{
...
print_r($items);
		...
		if ($this->_cart_contents[$items['rowid']]['qty'] == $items['qty'] AND ($this->_cart_contents[$items['rowid']]['options'] == $items['options']) )
		{
			return FALSE;
		}
    	...
    	if((isset($items['options']) AND is_array(($items['options'])) || (isset($items['options']))))
		{
